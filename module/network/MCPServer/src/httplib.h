...
namespace httplib {
    ...
    namespace detail {
        ...
        inline std::wstring u8string_to_wstring(const char* s) {
            ...
            if (wlen > 0) {
                ws.resize(wlen);
                wlen = ::MultiByteToWideChar(CP_UTF8,
                                             0,
                                             s,
                                             len,
    ...
    #define USE_IF2IP
        ...
        inline bool parse_header(const char* beg, const char* end, T fn) {
            ...
            if (p <= end) {
                ...
                auto val =
                    case_ignore::equal(key, "Location") ? std::string(p, end) : decode_url(std::string(p, end), false);
        ...
        bool read_content(Stream& strm,
            ...
            return prepare_content_receiver(
                ...
                [&](const ContentReceiverWithProgress& out) {
                    ...
                    if (!ret) {
                        status =
                            exceed_payload_max_length ? StatusCode::PayloadTooLarge_413 : StatusCode::BadRequest_400;
                    }
                    return ret;
                });
        }
 ...
 */
    ...
    inline Result Client::Get(const std::string& path,
                              ...
                              Progress progress) {
        return cli_
            ->Get(path, params, headers, std::move(response_handler), std::move(content_receiver), std::move(progress));
    }
